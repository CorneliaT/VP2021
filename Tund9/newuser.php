<?php
  ...
  if(isset($_POST["submitUserData"])){
	  ...
	  if(!empty($birthDay) and !empty($birthMonth) and !empty($birthYear)){
		  ...
		  //parooli kontroll (kas on olemas, kas vähemalt kaheksa tähte ja kaks korda ühtemoodi)
		  //strlen($_POST["password"]) > 8
		  
		  //kui kõik korras, siis salvestama
  
	  if(empty($nameError) and empty($birthMonthError) and empty($birthYearError) and empty($birthDayError) and empty($birthDateError) and empty($genderError) and empty($emailError) and empty($passwordError) and empty($confirmpasswordError)){
...
<html lang="et">
  ...
  <body>
	...
	<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	  ...
	  <input name="submitUserData" type="submit" value="Loo kasutaja"><span>
